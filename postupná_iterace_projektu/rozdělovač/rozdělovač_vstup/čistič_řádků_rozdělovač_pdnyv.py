...
filter_mask = (
    ~df[ZADANY_SLOUPEC_3].isin([
        KOD_PRACVMES_ARBITER,
        KOD_PRACVMES_FAU
    ])
) & (
    df[ZADANY_SLOUPEC_2] < LIMIT_PRO_OSCIS
)
...
save_with_footer(df_svatky_valid, "pdnyv_vystup_"+zpracovavany_mesic+"_svatky")

print("Export vikendů")
df_saturday_valid=has_valid_time(df_saturday)
save_with_footer(df_saturday_valid, "pdnyv_vystup_"+zpracovavany_mesic+"_soboty")
df_sunday_valid=has_valid_time(df_sunday)
save_with_footer(df_sunday_valid, "pdnyv_vystup_"+zpracovavany_mesic+"_nedele")
df_vikend_valid = pd.concat([df_saturday_valid, df_sunday_valid], ignore_index=False)
save_with_footer(df_vikend_valid, "pdnyv_vystup_"+zpracovavany_mesic+"_vikendy")
