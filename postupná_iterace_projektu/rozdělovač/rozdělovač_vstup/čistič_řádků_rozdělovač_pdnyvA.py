...
for month, days in SVATKY.items():
    holiday_mask |= (
        (df[ZADANY_SLOUPEC_1].dt.month == month)
        &
        (df[ZADANY_SLOUPEC_1].dt.day.isin(days))
    )

df_svatky = df.loc[holiday_mask]
"""
def get_holidays(df):
...
"""
df_saturday = df[df[ZADANY_SLOUPEC_1].dt.weekday == SATURDAY]
df_sunday = df[df[ZADANY_SLOUPEC_1].dt.weekday == SUNDAY]
...
def main():
    ...
    df_clean["kategorie"] = (df_clean[FILTER_ZAZNAMU_A].map(CATEGORY_MAP).fillna("droby")
    )
