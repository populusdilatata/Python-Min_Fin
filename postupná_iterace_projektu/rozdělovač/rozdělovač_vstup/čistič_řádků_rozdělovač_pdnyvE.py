...
def get_holidays(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Aktuálně jsou podporovány pevné svátky
    definované přímo ve funkci.
    ...
    holiday_mask = (
        ((df[COL.den].dt.month == 5) &
         (df[COL.den].dt.day.isin([1, 8])))
        |
        ((df[COL.den].dt.month == 7) &
         (df[COL.den].dt.day.isin([5, 6])))
    )
...
def save_with_footer(df_input: pd.DataFrame, base_filename: str) -> None:
    ...
    df_copy.loc[:, COL.den] = ( df_copy[COL.den].dt.strftime("%d.%m.%Y") )
...
def main():
    ...
    save_with_footer(
        df_clean,
        "pdnyv_porovnat_"+CFG.zpracovavany_mesic
    )
    """
