...
IGNOROVANE_LISTY: Final[set[str]] = {
    "Rel_zmena","Top5_pct", "WARNING_MZDY0", "WARNING-kontrola kompletnosti",
}
...
def zapis_warning_mzdy(
    ...
    warning_sloupce = [
        config.sloupec_id,
        config.sloupec_obdobi,
        "pracvz",
        "prijm",
        config.sloupec_hmzda,
...
def zapis_inc_dec_listy(
    ...
    sloupce_k_odstraneni = ["odmm", "odmn", "odmo", "odmr", ] + [f"odm{i}" for i in range(1, 21)]

    if df_top.empty:
        return

    for obdobi in sorted(
        df_top[config.sloupec_obdobi].unique()
    ):

        inc = (
            df_top[
                (df_top[config.sloupec_obdobi] == obdobi)
                &
                (
                    df_top["typ"]
                    == "top5_pct_INCREASE"
            ...
            .drop(columns=["typ", *sloupce_k_odstraneni],
                  errors="ignore",)
            .sort_values(
                "Rel_zmena_%",
                ascending=False,
        ...
        inc = inc.reindex(
            columns=[
                c for c in poradi_sloupcu if c in inc.columns
            ]
        )

        dec = dec.reindex(
            columns=[
                c for c in poradi_sloupcu if c in dec.columns
            ]
        )
...
def export_excel(
    ...
    warning_kompletnost: pd.DataFrame, df_odejiti: pd.DataFrame, config: Config,
) -> None:
...
def vytvor_odejiti(
    ...
    """
    Vytvoří přehled odešlých zaměstnanců.

    Do výsledku zahrne zaměstnance,
    kteří se nachází v souboru odejiti.xlsx
    a současně jsou:

    - v WARNING-kontrola kompletnosti,
    - nebo v některém z INC_* listů.

    Zaměstnanci z WARNING-kontrola kompletnosti
    jsou uvedeni pouze jednou.
    """
    ...
    if not warning_kompletnost.empty:
        ...
        warning_df = dopln_predchozi_hodnoty(
            warning_df, config.sloupec_id, config.sloupec_hmzda, config.sloupec_hodnoty,
        )
    ...
    # =====================================
    # průnik s odešlými zaměstnanci
    # =====================================

    odejiti_set = set(
        odejiti[config.sloupec_id].dropna()
    )
    ...
    poradi_sloupcu = [
        config.sloupec_id, config.sloupec_obdobi,"pracvz",
        "prijm", "hmzda_predchozi", config.sloupec_hmzda,
        "hmzda_odmen_predchozi", config.sloupec_hodnoty, "Rel_zmena",
        "Rel_zmena_%","puvod",
    ]

    vysledek = vysledek.reindex(
        columns=[
            c
...
def vytvor_boxplot(
    ...
    for obd in config.obdobi_boxplot:

        hodnoty = (
            df.loc[
                df[config.sloupec_obdobi] == obd,
                "Rel_zmena_%",
            ]
            .dropna()
        )
    ...
    logger.info("Boxplot vytvořen pro období: %s",", ".join(platna_obdobi),)
...
def priprav_data(
    ...
    return (
        df, warning_mzdy, warning_kompletnost,
    )
...
def main() -> None:
    ...
    try:
        ...
        (
            df, warning_mzdy, warning_kompletnost,) = priprav_data(df, config,)
        ...
        export_excel( df_export, df_top, warning_mzdy, warning_kompletnost, df_odejiti, config)
