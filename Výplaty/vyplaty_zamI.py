...
Excelový soubor vyplaty_zam07B.xlsx
obsahující sloučená historická a nově načtená data.
...
class Config:
    ...
    kontrolovana_obdobi: tuple[str, ...] = (
        "4.26", "5.26", "6.26","7.26", "8.26"
    )

    sloupce_odmen: tuple[str, ...] = (
        "odmn",
        "odmm",
        "odmo",
        "odmr",
    ...
    zachovane_sloupce: tuple[str, ...] = (
        ...
        "odmo",
        "odmr",
        *tuple(f"odm{i}" for i in range(1, 21)),
        "hmzda",
        "appvdr",
    )
...
CONFIG = Config(
    soubor="Výplaty/vyplaty_zam07_row.xlsx"
)
...
def priprav_sloupce(
    ...
    """
    Zachová pouze požadované sloupce
    a vytvoří hmzda_odmen.
    """
    ...
    dostupne = [
        sl
        for sl in config.zachovane_sloupce
        if sl in df.columns
    ]

    df = df[dostupne]

    for sl in config.sloupce_odmen:
...
@mer_cas
def vytvor_vyplaty07B(
    stara_data: pd.DataFrame,
    #nova_data: pd.DataFrame,
) -> pd.DataFrame:
    """
    Sloučí historická a nová data,
    uloží je do vyplaty_zam07B.xlsx
    a vrátí sloučený DataFrame.
    """
    ...
    soubor_vystup = Path(
        "Výplaty/vyplaty_zam07B.xlsx"
    )

    vysledek = vysledek.copy()
    ...
    logger.info(
        "vysledek období:\n%s",
        vysledek["obdobi"].value_counts(dropna=False)
    )
...
def kontrola_pracvz(
    ...
    agregace = (
        ...
        .agg(
            POCET_PRACVZ="nunique",
            SEZNAM_PRACVZ=lambda x: ", ".join(
                sorted(
    ...
    problematicti = agregace[
        agregace["POCET_PRACVZ"] > 1
    ]

    if problematicti.empty:
        return pd.DataFrame()
...
def kontrola_kompletnosti(
    ...
    problematicti["CHYBEJICI_OBDOBI"] = (
        problematicti["OBDOBI_SET"]
        .apply(
            lambda x: ", ".join(
                sorted(pozadovana_obdobi - x)
            )
        )
    )

    problematicti["NAVIC_OBDOBI"] = (
        problematicti["OBDOBI_SET"]
        .apply(
            lambda x: ", ".join(
                sorted(x - pozadovana_obdobi)
            )
        )
    )
...
def priprav_inc_odchody(
    ...
    if df_top.empty or "typ" not in df_top.columns:
        return pd.DataFrame()

    inc_df = (
        df_top[
            df_top["typ"] == "top5_pct_INCREASE"
        ]
        .copy()
    )
...
def priprav_data(
    ...
    Čtvrtá položka:
            Záznamy vyřazené z důvodu změny pracoviště.
    ...
    logger.info(
        "Po odstranění nulových mezd:\n%s",
        df["obdobi"].value_counts(dropna=False)
    )
...
def main() -> None:
    ...
    try:
        ...
        stara_data = nacti_data("Výplaty/vyplaty_zam_08_26B.xlsx")
        stara_data = priprav_sloupce(stara_data, config)

        #nova_data = nacti_data("Výplaty/vyplaty_zam_08_26.xlsx")
        #nova_data = priprav_sloupce(nova_data, config)

        logger.info(
                "stara_data období:\n%s",
                stara_data["obdobi"].value_counts(dropna=False)
        )

        #logger.info(
        #    "nova_data období:\n%s",
        #    nova_data["obdobi"].value_counts(dropna=False)
        #)

        df = vytvor_vyplaty07B(
                stara_data,
                #nova_data,
            )
