...
class Config:
    ...
    #===========================
    zpracovavany_mesic: str ="07_26"
    #=========================== 
    input_file: Path  = Path("vstup_pdnyv/07_DATA/pdnyv_07A.xlsx")
    base_output_dir: Path  = Path("porovnavač")
    #=========================== 
    zpracovavany_rok: int = 2026
    zpracovavany_mesic_cislo: int = 7
CFG = Config()
...
def create_empty_priche_summary(
    df_empty: pd.DataFrame
) -> pd.DataFrame:
    """
    Vytvoří souhrn počtu výskytů chybějícího
    příchodu nebo odchodu za jednotlivé pracovníky.
    """

    return (
        df_empty
        .groupby([COL.oscis, COL.prijm], dropna=False)
        .size()
        .reset_index(name="pocet_vyskytu")
        .sort_values(
            "pocet_vyskytu",
            ascending=False
        )
...
def get_empty_priche_odche(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # řádky s chybějícím příchodem nebo odchodem
    df_empty = df_month.loc[
        df_month[COL.priche].isna()
        |
        df_month[COL.odche].isna()
    ]

    # počet takových řádků za každého pracovníka
    counts = (
        df_empty.groupby(COL.oscis)
        .size()
    )

    qualified_oscis = counts[
        counts >= CFG.min_empty_priche_count
    ].index

    return df_empty.loc[
        df_empty[COL.oscis].isin(qualified_oscis)
    ]
...
def export_empty_priche_odche(
    ...
    if df_empty.empty:
        logger.warning(
            "Přeskočeno: 25_DNI_BEZ_PRUCHODU"
        )
        return

    filename = (
        f"25_DNI_BEZ_PRUCHODU_"
        f"{CFG.zpracovavany_mesic}_"
        f"{time_suffix}.xlsx"
    )

    filepath = output_dir / filename

    df_detail = df_empty.copy()
    ...
    df_summary = create_empty_priche_summary(
        df_empty
    )
    ...
    logger.info(
        "Soubor 25_DNI_BEZ_PRUCHODU vytvořen."
    )
