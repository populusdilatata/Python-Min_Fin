...
print("Filter")
df = df[(~df[ZADANY_SLOUPEC_3].isin([KOD_PRACVMES_ARBITER, KOD_PRACVMES_FAU])) & (df[ZADANY_SLOUPEC_2] < LIMIT_PRO_OSCIS)]
...
# ===== květen =====
"""
df_may = df[df[ZADANY_SLOUPEC_1].dt.month == 5]

df_1_may = df_may[df_may[ZADANY_SLOUPEC_1].dt.day == 1]
df_8_may = df_may[df_may[ZADANY_SLOUPEC_1].dt.day == 8]

df_svatky = pd.concat([df_1_may, df_8_may], ignore_index=False)

# ===== červenec =====
df_jul = df[df[ZADANY_SLOUPEC_1].dt.month == 7]

df_6_jul = df_jul[df_jul[ZADANY_SLOUPEC_1].dt.day == 6]
df_7_jul = df_jul[df_jul[ZADANY_SLOUPEC_1].dt.day == 7]

df_svatky = pd.concat([df_6_jul, df_7_jul], ignore_index=False)
"""

#Neotestovaný kód 140-151
...
# filtr svátků
df_svatky = df[
    df[ZADANY_SLOUPEC_1].apply(
        lambda x: x.month in svatky and x.day in svatky[x.month]
    )
]
#Neotestovaný kód

df_saturday = df[df[ZADANY_SLOUPEC_1].dt.weekday == 5]
df_sunday = df[df[ZADANY_SLOUPEC_1].dt.weekday == 6]
...
def save_with_footer(df_input, base_filename):
    ...
    df_copy[ZADANY_SLOUPEC_1] = df_copy[ZADANY_SLOUPEC_1].dt.strftime("%d.%m.%Y")
...
save_with_footer(
    ...
    "pdnyv_vystup_FILTER_prichv_EMPTY_kveten"
)
...
#save_with_footer(df_1_may_valid, "pdnyv_vystup_1_kveten")
#df_8_may_valid =has_valid_time(df_8_may)
#save_with_footer(df_8_may_valid, "pdnyv_vystup_8_kveten")
df_svatky_valid =has_valid_time(df_svatky)
#df_svatky_valid = pd.concat([df_1_may_valid, df_8_may_valid], ignore_index=False)
save_with_footer(df_svatky_valid, "pdnyv_vystup_svatky_kveten")

print("Export vikendů")
df_saturday_valid=has_valid_time(df_saturday)
save_with_footer(df_saturday_valid, "pdnyv_vystup_soboty_kveten")
df_sunday_valid=has_valid_time(df_sunday)
save_with_footer(df_sunday_valid, "pdnyv_vystup_nedele_kveten")
df_vikend_valid = pd.concat([df_saturday_valid, df_sunday_valid], ignore_index=False)
save_with_footer(df_vikend_valid, "pdnyv_vystup_vikendy_kveten")
...
for cat in df_clean["kategorie"].unique():
    df_clean_split[cat] = df_clean[df_clean["kategorie"] == cat]


for name, df_part in df_clean_split.items():
    save_with_footer(
        df_part.drop(columns=["kategorie"]),  # volitelné
        f"pdnyv_meziclen_05_{name}"
    )
